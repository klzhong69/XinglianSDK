 ...
 * APP 端获取指定的日常活动数据
 */
public class SpecifySportsHistoryCall extends WriteCallback {
    ...
    public SpecifySportsHistoryCall(String address, long time, long endTime, BleWrite.SpecifySportsHistoryCallInterface mInterface) {
        super(address);
        mList = new ArrayList<>();
    ...
    public byte[] getSendData() {
        TLog.Companion.error("发送++"+ByteUtil.getHexString(CmdUtil.getPlayer("02",Config.BigData.APP_SPECIFY_SPORTS, HexDump.toByteArray(startTime))));
        return CmdUtil.getFullPackage(CmdUtil.getPlayer("02", Config.BigData.APP_SPECIFY_SPORTS, HexDump.toByteArray(startTime)));
    }
...
//        else {
            //正常数据的
            if (result[0]==Config.PRODUCT_CODE&&result[8] == Config.BigData.KEY &&result[9]== Config.BigData.DEVICE_SPECIFY_SPORTS) {
...
//                  if(data[9]==0x24) { //如果上层判断了 下层可以不用再次判断
                      ...
                      for (int i = 10; i < data.length; i += HexDump.byte2intHigh(data, i+1, 2)+3) {
                          TLog.Companion.error("==="+HexDump.byte2intHigh(data, i+1, 2));
                          mDataBean = new DataBean();
                          mDataBean.setType(data[i+3]);
                          mDataBean.setTotalSteps(HexDump.byte2intHigh(data, i+4, 4));
                          mDataBean.setDistance(HexDump.byte2intHigh(data, i + 8, 4));
                          mDataBean.setCalories(HexDump.byte2intHigh(data, i + 12, 4));
                          mDataBean.setAverageHeartRate(data[17]);
                          mDataBean.setAverageHeartRate(data[18]);
                          mDataBean.setAverageHeartRate(data[19]);
                          mList.add(mDataBean);
                      }
                    if (result[9]== Config.BigData.DEVICE_SPECIFY_SPORTS) {
                        TLog.Companion.error("怎么回事?");
