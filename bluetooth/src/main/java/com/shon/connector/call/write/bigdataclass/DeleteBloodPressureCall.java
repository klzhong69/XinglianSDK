 ...
 * APP 删除指定的心率数据
 */
public class DeleteBloodPressureCall extends WriteCallback {
    byte keyValue;
    long time = 0L;

    public DeleteBloodPressureCall(String address, long time) {
        super(address);
        this.time = time;
    }

    public DeleteBloodPressureCall(String address, byte keyValue) {
        super(address);
    ...
    public byte[] getSendData() {
        ...
        byte[] head = {Config.PRODUCT_CODE, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, HexDump.byteXOR(keyTime), 0x02, 0x0F, 0x00, 0x04};
