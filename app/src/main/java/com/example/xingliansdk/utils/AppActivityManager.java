...
public class AppActivityManager {
     ...
     * @param activitys
     */
    public final void finishActivitys(Class<? extends Activity>... activitys) {
        if (activitys == null || activitys.length == 0) return;
        for (Class<? extends Activity> itemActivity : activitys) {
            if (itemActivity != null)
                finishActivity(itemActivity);
        }
    }
    ...
    private void pushActivity(Activity activity) {
        TLog.Companion.logv("当前入栈activity ===>>>" + activity.getClass().getSimpleName());
        if (activityStack == null) {
            activityStack = new Stack<WeakReference<Activity>>();
        }
    ...
    private void removeActivity(Activity activity) {
        TLog.Companion.logv("当前出栈activity ===>>>" + activity.getClass().getSimpleName());
        for (WeakReference<Activity> weakReference : activityStack) {
