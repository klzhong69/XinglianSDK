...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawDataSet(Canvas c, ILineDataSet dataSet) {
        ...
        switch (dataSet.getMode()) {
            ...
            case LINEAR:
            case STEPPED:
                drawLinear(c, dataSet, false);
                break;
            case Y_ZERO:
                drawLinear(c, dataSet, true);
                break;
            case CUBIC_BEZIER:
            ...
            case HORIZONTAL_BEZIER:
                ...
                break;
            case HORIZONTAL_BEZIER_Y_ZERO:
                drawHorizontalBezierZero(dataSet);
            //    drawHorizontalBezier(dataSet, true);
                break;
        }
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet/*, boolean yZero*/) {
...
//                }
    ...
    protected void drawHorizontalBezierZero(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                if (prev.getY() == 0 && cur.getY() == 0) {
                    //上一个和当前都是0,继续循环
                    continue;
                } else if (prev.getY() == 0 && cur.getY() != 0) {
                    //上一个为0,当前不为0,表示开始新的线段
                    start = j;
                    ...
                    if (dataSet.isDrawFilledEnabled()) {
                        ...
                        cubicFillPath.addPath(cubicPath);
                        // create a new path, this is bad for performance
                        drawCubicFill1(mBitmapCanvas, dataSet, start, end, cubicFillPath, trans, mXBounds);
                    }
...
//         if filled is enabled, close the path
    ...
    protected void drawCubicFill1(Canvas c, ILineDataSet dataSet, int start, int end, Path spline, Transformer trans, XBounds bounds) {
        if (start >= end) {
            return;
        }
        float fillMin = dataSet.getFillFormatter()
                .getFillLinePosition(dataSet, mChart);
        spline.lineTo(dataSet.getEntryForIndex(end).getX(), fillMin);

        spline.lineTo(dataSet.getEntryForIndex(start).getX(), fillMin);
        spline.close();
     ...
     * @param dataSet
     */
    protected void drawLinear(Canvas c, ILineDataSet dataSet, boolean yZero) {
